<template>
  <div class="relative">
    <div
      class="relative mt-2 flex w-full flex-shrink-0 flex-col-reverse text-left"
    >
      <input
      ...
      <label
        ...
        :class="[
          invalid ? 'text-red-alert' : disabled ? 'text-gray' : 'text-gray-90',
        ]"
        :for="computedId"
      >
        {{ label }}
      ...
      <span
        v-if="$slots.append"
        class="absolute top-0 bottom-0 right-0 z-10 w-14"
      >
        <slot
          name="append"
          :disabled="disabled"
